/**
 * 20-link pendulum chain with real-time Qt visualization.
...
// ─────────────────────────────────────────────
struct Simulation {
    MultibodySystem sys{"Chain20"};

    std::shared_ptr<RigidBody> ground;
    std::vector<std::shared_ptr<RigidBody>> links;
    std::vector<std::shared_ptr<SphericalJoint>> joints;
    ...
    void build() {
        ...
        for (int i = 0; i < N_LINKS; i++) {
            ...
            auto jt = std::make_shared<SphericalJoint>(
                prevBody, link.get(),
                prevLocal, curLocal,
                "J" + std::to_string(i));
...
private:
    ...
    void drawHUD(QPainter& p, int w, int /*h*/) {
        ...
        line(QString("%1 link zincir").arg(N_LINKS));
